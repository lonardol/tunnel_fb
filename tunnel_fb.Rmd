...
ldata<-read.csv("data/Tunnel_FB_latency.csv", header = TRUE) %>% 
  ...
  filter(X!=4 & X!=5) %>%  #remove extra coding of alredy coded subject (Kanai)
...
p.latency <- ggplot(data = plot.latency.data, aes(x = condition, y = mean.latency)) +
  ...
  #ylim(0.2,0.6)+
  theme_classic() +
  labs(x = "Experimental group", y = "Latency to approach the tunnel")+
  ggtitle("All breeds (N=63)")

p.latency
```
# Check howoften dogs switched choice after tunnel
```{r}
all.data <- all.data %>%
...
all.data<-all.data %>% 
    ...
    breed == "wolfspitz" | breed == "spitz" ~ "german_spitz", #LL: I am assuming spitz was also German spitz
